...
describe("GitHub Webhook Signature Validation", () => {
	it("validates a correct GitHub signature", () => {
	...
	it("rejects an invalid GitHub signature", () => {
		...
		expect(
			verifyGithubWebhookSignature(payload, "sha256=invalid-signature", secret),
		).toBe(false);
	});

	it("rejects webhook when GitHub secret is not configured", () => {
		const isValid = validateGithubWebhookRequest({
			rawBody: Buffer.from("{}"),
...
describe("GitLab/Gitea Webhook Validation", () => {
	it("validates GitLab token header with matching secret", () => {
		expect(
			validateGitlabWebhookRequest({
	...
	it("rejects GitLab token when missing or invalid", () => {
		expect(
			validateGitlabWebhookRequest({
	...
	it("validates Gitea signature when secret and signature are correct", () => {
		const rawBody = Buffer.from(JSON.stringify({ ref: "refs/heads/main" }));
		const signature = crypto
			.createHmac("sha256", "gitea-secret")
			.update(rawBody)
			.digest("hex");
	...
	it("rejects Gitea signature when secret is missing", () => {
		expect(
			validateGiteaWebhookRequest({
				rawBody: Buffer.from("{}"),
				signatureHeader: "abc",
				giteaWebhookSecret: null,
				resourceId: "app-1",
			}),
		).toBe(false);
	});
