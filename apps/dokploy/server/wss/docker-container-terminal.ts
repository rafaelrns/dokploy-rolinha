...
export const setupDockerContainerTerminalWebSocketServer = (
	...
	wssTerm.on("connection", async (ws, req) => {
		...
		if (!containerId) {
			console.warn("[wss-docker-terminal] Rejected connection: missing containerId");
		...
		if (!isValidContainerId(containerId)) {
			console.warn(
				`[wss-docker-terminal] Rejected invalid containerId: ${containerId}`,
			);
		...
		if (activeWay && !isValidShell(activeWay)) {
			console.warn(
				`[wss-docker-terminal] Rejected invalid shell: ${activeWay}`,
			);
		...
		if (!user || !session) {
			console.warn("[wss-docker-terminal] Rejected unauthenticated connection");
			ws.close();
			return;
		}
		try {
