...
const SHOULD_USE_SECURE_COOKIES_IN_SELF_HOSTED =
	process.env.NODE_ENV === "production" && !ALLOW_INSECURE_COOKIES;

if (process.env.NODE_ENV === "production" && ALLOW_INSECURE_COOKIES) {
	console.warn(
		"[auth] Running in production with ALLOW_INSECURE_COOKIES=true. Session cookies may be exposed over HTTP.",
	);
}
...
const { handler, api } = betterAuth({
	...
	...(!IS_CLOUD
		? {
				advanced: {
					useSecureCookies: SHOULD_USE_SECURE_COOKIES_IN_SELF_HOSTED,
					defaultCookieAttributes: {
						sameSite: "lax",
						secure: SHOULD_USE_SECURE_COOKIES_IN_SELF_HOSTED,
						httpOnly: true,
						path: "/",
					},
				},
