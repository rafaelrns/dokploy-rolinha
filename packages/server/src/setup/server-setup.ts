...
export const installRClone = () => `
    if command_exists rclone; then
		...
		INSTALLER_PATH=$(mktemp /tmp/dokploy-rclone-installer-XXXXXX.sh)
		download_and_verify "https://rclone.org/install.sh" "$INSTALLER_PATH" "$RCLONE_INSTALLER_SHA256"
		$SUDO_CMD bash "$INSTALLER_PATH"
		rm -f "$INSTALLER_PATH"
...
const installNixpacks = () => `
	if command_exists nixpacks; then
		...
		INSTALLER_PATH=$(mktemp /tmp/dokploy-nixpacks-installer-XXXXXX.sh)
		download_and_verify "https://nixpacks.com/install.sh" "$INSTALLER_PATH" "$NIXPACKS_INSTALLER_SHA256"
		$SUDO_CMD bash "$INSTALLER_PATH"
		rm -f "$INSTALLER_PATH"
...
const installRailpack = () => `
	if command_exists railpack; then
		...
		INSTALLER_PATH=$(mktemp /tmp/dokploy-railpack-installer-XXXXXX.sh)
		download_and_verify "https://railpack.com/install.sh" "$INSTALLER_PATH" "$RAILPACK_INSTALLER_SHA256"
		$SUDO_CMD bash "$INSTALLER_PATH"
		rm -f "$INSTALLER_PATH"
...
const installBuildpacks = () => `
	...
	if command_exists pack; then
		...
		ARCHIVE_PATH=$(mktemp /tmp/dokploy-buildpacks-XXXXXX.tgz)
		download_and_verify "https://github.com/buildpacks/pack/releases/download/v$BUILDPACKS_VERSION/pack-v$BUILDPACKS_VERSION-linux$SUFFIX.tgz" "$ARCHIVE_PATH" "$BUILDPACKS_ARCHIVE_SHA256"
		$SUDO_CMD tar -C /usr/local/bin/ --no-same-owner -xzvf "$ARCHIVE_PATH" pack
		rm -f "$ARCHIVE_PATH"
