...
const ENABLE_INSECURE_TRAEFIK_API =
	process.env.DOKPLOY_ENABLE_INSECURE_TRAEFIK_API === "true";

if (ENABLE_INSECURE_TRAEFIK_API) {
	console.warn(
		"[security] DOKPLOY_ENABLE_INSECURE_TRAEFIK_API=true exposes Traefik dashboard/API without authentication.",
	);
}
...
export const getDefaultTraefikConfig = () => {
	const configObject: MainTraefikConfig = {
		...
		api: {
			insecure: ENABLE_INSECURE_TRAEFIK_API,
		},
		...(process.env.NODE_ENV === "production" && {
