...
export const getRailpackCommand = (application: ApplicationNested) => {
	...
	const buildMemoryLimit =
		process.env.DOKPLOY_BUILD_MEMORY_LIMIT || "2147483648";
	const buildCpuLimit = process.env.DOKPLOY_BUILD_CPU_LIMIT || "2";
	const buildPidsLimit = process.env.DOKPLOY_BUILD_PIDS_LIMIT || "512";
	// Build command
	const buildArgs = [
		"buildx",
		"build",
