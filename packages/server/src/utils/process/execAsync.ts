...
export const getBuildTimeoutSeconds = () => {
	const value = Number.parseInt(
		process.env.DOKPLOY_BUILD_TIMEOUT_SECONDS || "1800",
		10,
	);
	return Number.isFinite(value) && value > 0 ? value : 1800;
};

export const withShellTimeout = (command: string, timeoutSeconds: number) => {
	if (!Number.isFinite(timeoutSeconds) || timeoutSeconds <= 0) {
		return command;
	}

	const quotedCommand = quote([command]);
	return `if command -v timeout >/dev/null 2>&1; then timeout --signal=TERM --kill-after=30s ${timeoutSeconds}s sh -lc ${quotedCommand}; else sh -lc ${quotedCommand}; fi`;
};
