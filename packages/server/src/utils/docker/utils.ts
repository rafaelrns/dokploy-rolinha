...
type ResourceProfile = "application" | "database";

const DEFAULT_RESOURCE_PROFILES: Record<
	ResourceProfile,
	{
		memoryLimit: string;
...
> = {
	application: {
		memoryLimit: process.env.DOKPLOY_DEFAULT_APP_MEMORY_LIMIT || "536870912",
		memoryReservation:
			process.env.DOKPLOY_DEFAULT_APP_MEMORY_RESERVATION || "268435456",
		cpuLimit: process.env.DOKPLOY_DEFAULT_APP_CPU_LIMIT || "500000000",
		cpuReservation:
			process.env.DOKPLOY_DEFAULT_APP_CPU_RESERVATION || "250000000",
	},
	database: {
		memoryLimit: process.env.DOKPLOY_DEFAULT_DB_MEMORY_LIMIT || "1073741824",
		memoryReservation:
			process.env.DOKPLOY_DEFAULT_DB_MEMORY_RESERVATION || "536870912",
		cpuLimit: process.env.DOKPLOY_DEFAULT_DB_CPU_LIMIT || "1000000000",
		cpuReservation:
			process.env.DOKPLOY_DEFAULT_DB_CPU_RESERVATION || "500000000",
	},
};

const toDockerResourceNumber = (value?: string | null) => {
...
export const calculateResources = ({
	...
	cpuLimit,
	cpuReservation,
}: Resources, profile: ResourceProfile = "application"): ResourceRequirements => {
	const defaults = DEFAULT_RESOURCE_PROFILES[profile];

	return {
		Limits: {
			MemoryBytes:
				toDockerResourceNumber(memoryLimit) ??
				toDockerResourceNumber(defaults.memoryLimit),
			NanoCPUs:
				toDockerResourceNumber(cpuLimit) ??
				toDockerResourceNumber(defaults.cpuLimit),
		},
		Reservations: {
			MemoryBytes:
				toDockerResourceNumber(memoryReservation) ??
				toDockerResourceNumber(defaults.memoryReservation),
			NanoCPUs:
				toDockerResourceNumber(cpuReservation) ??
				toDockerResourceNumber(defaults.cpuReservation),
		},
	};
};
