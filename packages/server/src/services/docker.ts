...
const CONTAINER_ID_REGEX = /^[a-zA-Z0-9.\-_]+$/;
const SAFE_CONTAINER_PATH_REGEX = /^[a-zA-Z0-9._/\-]+$/;

export const normalizeContainerDestinationPath = (destinationPath: string) => {
	const trimmedPath = destinationPath.trim();
	if (!trimmedPath) {
		console.warn("[security] Rejected empty destination path for container upload");
	...
	if (
		...
		!SAFE_CONTAINER_PATH_REGEX.test(normalizedPath)
	) {
		console.warn(
			`[security] Rejected unsafe destination path for container upload: ${normalizedPath}`,
		);
		throw new Error("Invalid destination path");
	}
...
export const uploadFileToContainer = async (
	...
	if (!CONTAINER_ID_REGEX.test(containerId)) {
		console.warn(
			`[security] Rejected invalid container ID format in uploadFileToContainer: ${containerId}`,
		);
		throw new Error("Invalid container ID");
	}

	const normalizedPath = normalizeContainerDestinationPath(destinationPath);

	const base64Content = fileBuffer.toString("base64");
	const tempFileName = `dokploy-upload-${Date.now()}-${fileName.replace(/[^a-zA-Z0-9.-]/g, "_")}`;
	const tempPath = `/tmp/${tempFileName}`;
	const quotedTempPath = quote([tempPath]);
	const quotedContainerDestination = quote([`${containerId}:${normalizedPath}`]);
	const command = `echo '${base64Content}' | base64 -d > ${quotedTempPath} && docker cp ${quotedTempPath} ${quotedContainerDestination} && rm -f ${quotedTempPath}`;
