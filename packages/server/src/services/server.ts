...
export const findServerByIdForOrganization = async (
	serverId: string,
	organizationId: string,
) => {
	const currentServer = await findServerById(serverId);

	if (currentServer.organizationId !== organizationId) {
		console.warn(
			`[authz] Rejected server access for serverId=${serverId} requestedOrg=${organizationId} actualOrg=${currentServer.organizationId}`,
		);
		throw new TRPCError({
			code: "UNAUTHORIZED",
