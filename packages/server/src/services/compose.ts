...
const executeBuildStepWithLogs = async (
	command: string,
	logPath: string,
	serverId?: string | null,
) => {
	const commandWithLog = `(${command}) >> ${logPath} 2>&1`;
	const timedCommand = withShellTimeout(commandWithLog, getBuildTimeoutSeconds());

	if (serverId) {
		await execAsyncRemote(serverId, timedCommand);
	} else {
		await execAsync(timedCommand);
	}
};
...
export const deployCompose = async ({
	...
	try {
		...
		await executeBuildStepWithLogs(
			command,
			deployment.logPath,
			compose.serverId,
		);
		...
		await executeBuildStepWithLogs(command, deployment.logPath, compose.serverId);
