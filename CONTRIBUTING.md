...
```bash
# Install Nixpacks
curl -fsSLo /tmp/nixpacks-install.sh https://nixpacks.com/install.sh \
...
```bash
# Install Railpack
curl -fsSLo /tmp/railpack-install.sh https://railpack.com/install.sh \
    && chmod +x /tmp/railpack-install.sh \
    && /tmp/railpack-install.sh
```

```bash
# Install Buildpacks
curl -fsSLo /tmp/pack-v0.39.1-linux.tgz "https://github.com/buildpacks/pack/releases/download/v0.39.1/pack-v0.39.1-linux.tgz" \
    && tar -C /usr/local/bin/ --no-same-owner -xzvf /tmp/pack-v0.39.1-linux.tgz pack
```
